...
<div id='std_list'>
	...
	<ul>
		...
		<c:forEach var='v' items='${list }' varStatus='status'>
			...
			<li class='item' onclick = "view('${v.id}')"> <!-- Id는 문자라 v.getId()를 ''로 감싸야 한다. --> 
		...
		<li class='btn_page'> <!-- page이동 버튼 -->
		  ...
		  <!-- 현재 보고있는 페이지가 첫페이지일때 처음,이전버튼 없애기 -->
		...
		<input type='button' value='${i }' onclick='movePage(${i})'/>
		 ...
		 <c:if test="${pageVo.totPage gt pageVo.endPage }"><!-- 현재 보고있는 페이지가 마지막일 때 다음,맨끝버튼 없애기 -->	
